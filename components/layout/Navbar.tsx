  ...
  return (
    <header
      className={`fixed top-0 z-[100] w-full transition-all duration-500 ${isSolid
        ? "bg-black/95 backdrop-blur-2xl shadow-2xl border-b border-white/5 py-0"
        : "bg-transparent py-4"
        }`}
    >
      <div className="container mx-auto px-6">
        ...
        shadow-lg shadow-black/40 overflow-hidden group-hover:scale-110 transition-all duration-500">
          ...
          group-hover:text-yellow-400 transition duration-300">
              ...
              <span className="text-yellow-500 text-[10px] uppercase tracking-[0.35em] mt-1 opacity-80">
            ...
            {navLinks.map((link) => (
              ...
              >
                {link.children ? (
                  ...
                  <Link
                    href={link.href}
                    className={`text-[11px] font-bold uppercase tracking-[0.2em] py-2 px-4 rounded-full transition-all duration-300 ${pathname === link.href ? "text-gold bg-white/5" : "text-gray-300 hover:text-white"
                      }`}
                  >
                    {link.label}
                  </Link>
                ...
                {link.children && dropdown === link.href && (
                  <div
                    ...
                    onMouseLeave={handleMouseLeave}
                  >
                    <div className="bg-dark/95 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-fade-in-up p-2">
          ...
          {/* Mobile Menu Toggle */}
          <button
            className="lg:hidden text-white p-3 hover:bg-white/5 rounded-full transition-colors"
      ...
      {/* Mobile Menu Drawer */}
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 top-[80px] bg-black/95 backdrop-blur-2xl z-40 overflow-y-auto animate-fade-in-up">
          <nav className="container mx-auto px-6 py-12 flex flex-col gap-6">
            {navLinks.map((link) => (
              <div key={link.href} className="flex flex-col gap-4">
                {link.children ? (
                  <div className="flex flex-col gap-2">
                    <span className="text-gold text-[10px] font-bold uppercase tracking-widest opacity-60">
                      {link.label}
                    </span>
                    <div className="flex flex-col gap-1 pl-4 border-l border-white/10 mt-2">
                      {link.children.map(child => (
                        <Link
                          key={child.href}
                          href={child.href}
                          className={`py-3 text-sm font-medium ${pathname === child.href ? 'text-white' : 'text-gray-400'}`}
                  ...
                  <Link
                    href={link.href}
                    className={`text-2xl font-serif border-b border-white/5 pb-4 ${pathname === link.href ? 'text-gold' : 'text-white'}`}
                    onClick={() => setMobileOpen(false)}
                  >
                    {link.label}
                  </Link>
